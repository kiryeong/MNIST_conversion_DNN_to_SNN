# -*- coding: utf-8 -*-
"""
Created on Tue Feb  2 11:21:24 2021
...
# 가중치 확인
w_input_hidden = model.layers[1].get_weights()
print('input-hidden layer weights:')
print(w_input_hidden)            

w_hidden_output = model.layers[2].get_weights()
print('hidden-output layer weights:')
print(w_hidden_output)

pickle.dump(w_input_hidden, open("w_data1.p", "wb"))
pickle.dump(w_hidden_output, open("w_data2.p", "wb"))

model2 = keras.models.Sequential([
     keras.layers.Flatten(input_shape = (28,28)), # 입력 이미지의 크기가 28X28 이므로 이를 1차원 텐서로 펼치는 것                                          
     keras.layers.Dense(N2, activation = 'relu', use_bias=False, weights=model.layers[1].get_weights())])
